...
OLD_CORNER_NAMES = ("a", "b", "c", "d")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Visualize the 24 bottom-to-bottom corner alignment cases.")
...
def generate_alignment_cases(
    ...
    new_width: float,
    new_height: float,
) -> tuple[list[AlignmentCase], tuple[float, float, float], tuple[float, float, float]]:
    ...
    for yaw, size_x, size_y in (
        ...
        for old_index, old_corner in enumerate(old_world_corners):
            for new_index, new_corner in enumerate(local_corners):
                if new_index == old_index:
                    continue
                x_min = old_corner[0] - new_corner[0]
                y_min = old_corner[1] - new_corner[1]
                cases.append(
                    AlignmentCase(
                        index=case_index,
                        yaw=yaw,
                        old_corner_name=OLD_CORNER_NAMES[old_index],
                        new_corner_name=new_corner_names[new_index],
                        old_corner=old_corner,
                        new_corner=new_corner,
                        new_center=(x_min + size_x / 2, y_min + size_y / 2, new_height / 2),
...
class BottomAlignmentViewer(world_view.World):
    def __init__(
        self,
        cases: list[AlignmentCase],
    ...
    def _show_case(self, case_index: int) -> None:
        ...
        marker_pos = np.array([case.old_corner[0], case.old_corner[1], self.pallet_thickness + 0.01], dtype=np.float32)
    ...
    def _update_status(self) -> None:
        ...
        self.status_label.text = (
            f"case {case.index}/24 | yaw={yaw_text} | "
            f"{case.old_corner_name} > {case.new_corner_name} | "
            "space/right: next, left: prev"
        )
        self.set_caption(f"Bottom Alignment Demo | case {case.index}/24")
...
def main() -> None:
    ...
    cases, old_center, old_size = generate_alignment_cases(
    ...
    cam_pos = (1.2, -1.1, 1.2)
    cam_lookat = (0.0, 0.0, args.pallet_thickness + max(args.old_height, args.new_height) * 0.5)
    viewer = BottomAlignmentViewer(
        cases=cases,
