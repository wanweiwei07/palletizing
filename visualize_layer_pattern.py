...
from palletizing import LayerPlacement, PalletSpec, load_task_boxes, plan_best_layer_pattern
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Visualize a single best pallet layer pattern.")
...
def _color_for_block(placement: LayerPlacement) -> np.ndarray:
    if placement.is_composite:
        return const.Tab20.ORANGE_LIGHT
    return const.Tab20.BLUE_LIGHT


def _make_block_object(placement: LayerPlacement, pallet_thickness: float):
    center = np.array(
        [placement.x, placement.y, placement.target_height / 2 + pallet_thickness],
        dtype=np.float32,
    )
    ...
    half_extents = np.array(
        [local_size_x / 2, local_size_y / 2, placement.target_height / 2],
        dtype=np.float32,
    )
    ...
    return prim.box(
        ...
        rotmat=rotmat,
        rgb=_color_for_block(placement),
        alpha=1.0,
    )
...
class AnimatedLayerViewer(world_view.World):
    def __init__(
        self,
        ...
        self.used_box_count = used_box_count
    ...
    def _advance_animation(self, dt: float) -> None:
        if self.paused or self.current_index >= self.total:
            self._update_status()
            return
        self._add_next_block()

    def _add_next_block(self) -> None:
        if self.current_index >= self.total:
            self._update_status()
            return
        placement = self.placements[self.current_index]
        _make_block_object(placement, self.pallet_thickness).attach_to(self.scene)
    ...
    def _update_status(self) -> None:
        ...
        self.status_label.text = (
            f"blocks: {self.current_index}/{self.total} | "
            f"target_h: {self.target_height:.2f} | "
            f"used_boxes: {self.used_box_count} | "
            f"u2d: {self.utilization_2d:.3f} | "
            f"state: {state} | "
            "space: pause/resume, right: next, r: restart"
        )
        self.set_caption(f"Layer Pattern | shown {self.current_index}/{self.total}")
...
def main() -> None:
    ...
    result = plan_best_layer_pattern(boxes, pallet, max_stack_size=args.max_stack_size)
    stack_span = args.pallet_thickness + result.target_height
    cam_pos = (pallet.length * 1.7, -pallet.width * 1.5, stack_span * 2.6)
    cam_lookat = (pallet.length / 2, pallet.width / 2, stack_span / 2)
    viewer = AnimatedLayerViewer(
        ...
        used_box_count=result.used_box_count,
